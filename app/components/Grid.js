...
const Grid = () => {
    ...
    const items = [
        { id: 1, title: "Glock 19: Semi-automatic pistol with preinstalled suppresor", image: 'https://th.bing.com/th/id/OIP.XAjblCZuiOJOUcr8SJsvOQAAAA?rs=1&pid=ImgDetMain', price: "$400", reviews: 5, sellerName: 'The Smuglers', sellerId: 'None', description: dummy, category: "Arms and Ammunation" },
                        ...
                        {items.map(item => {
                            return (
                                ...
                                <Link key={item.id} href={{ pathname: '/items/' + item.id, query: { title: item.title, id: item.id, image: item.image, price: item.price, reviews: item.reviews, sellerName: item.sellerName, sellerId: item.sellerId, description: item.description, } }}
                                    className="lg:w-1/4 mr-2  rounded-lg md:w-1/2 p-2 w-full hover:scale-[1.01] transition-all duration-150 ease-out" bis_skin_checked="1">

                                    <img alt="ecommerce" className="object-fill  object-center w-full rounded-lg " src={item.image} />
